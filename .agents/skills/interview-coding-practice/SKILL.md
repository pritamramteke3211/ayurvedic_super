...
Sprint coding **42** (01+02+03) & Sprint+PartC **61** are **100% COMPLETED**. Tracker tables show **Overall Coding (Part B)**, **Overall Part C**, and **Overall Combined** daily pending pace over remaining days left through 21 Aug.
...
5. **Mandatory Tracker Refresh on Open/Review/Next:** Across `PROGRESS.md` and `CURRENT.md` (and updating workspace status in `my-answer.md` when reviewed). Keep global trackers updated.
...
2. **Primary Code Section & History Below:** The main solution section MUST remain as `## Code` (for Part B) or `- **Fix:**` (for Part C) with `tsx ... `. Always read and check the user's answer directly from `## Code` / `- **Fix:**`. If an attempt does not satisfy requirements (`needs_retry`), preserve a snapshot of the failed attempt below under `## Attempt History` (`### Try 1`, `### Try 2`), placed **below `## Notes (for me)`** in `my-answer.md`, while keeping the user's attempted code in `## Code` / `- **Fix:**` so they can directly refine it for the active attempt without re-typing from scratch. Record attempt counts (`tries: N`) in `CURRENT.md` and `PROGRESS.md`. Never overwrite previous history logs.
...
7. **Always Pre-populate Code Fences & Preserve Attempts in `my-answer.md`:** When scaffolding, creating, or opening `my-answer.md` for Part B coding or Part C, ALWAYS pre-populate empty ` ```tsx ` (or ` ```ts `) code blocks under `## Code` (for Part B coding) and under `- **Fix:**` (for Part C). On `needs_retry`, keep the user's code attempt inside `- **Fix:**` or `## Code` so they can edit it in place for their next try. Never leave `- **Fix:**` or `## Code` without a code block ready for the user to type or edit.

### Part A theory — read + discuss + append

1. **OK to read** `question.md` (source Explanation/Example/Note are included in `question.md`).
...
3. When user is **confused** about a theory answer:
   ...
   - **Append** a dated entry under `question.md` → **Discussion / appended explanations** (do not only leave it in chat).
...
5. Do not treat reading `question.md` as mastery.
...
| Practice theory / Section N theory | Open next `unread` theory (or named T##); point to `question.md` |
| Confused about … / explain theory | Teach + **append** to that theory `question.md` Discussion |
...
2. Folder has single `question.md` containing prompt, answer, and Discussion section.
3. Point user to `question.md`; invite discussion of confusing lines.
...
When clarifying, append to `question.md`:
...
| Path                             | Role                                        |
| -------------------------------- | ------------------------------------------- |
| `…/theory/T##/question.md`       | Theory prompt + Answer + Discussion appends |
| `…/coding/C##/`                  | Part B                                      |
| `…/predict-output/` or `part-c/` | Part C                                      |
| `12-senior-system-design/`       | Design                                      |
| `_templates/theory-question.md`  | Theory template                             |
